...
class ReminderSettings(BaseModel):
    enabled: bool = True
    days_before: int = 1  # Send reminder X days before event
    event_types: List[str] = ["court_date", "attorney_meeting", "visitation", "medical"]
...
async def get_reminder_settings(current_user: dict = Depends(get_current_user)):
    ...
    if not settings:
        ...
        return {
            ...
            "enabled": True,
            "days_before": 1,
            "event_types": ["court_date", "attorney_meeting", "visitation", "medical"]
        }
    return settings

@api_router.put("/reminders/settings")
async def update_reminder_settings(settings: ReminderSettings, current_user: dict = Depends(get_current_user)):
...
async def send_event_reminders(current_user: dict = Depends(get_current_user)):
    """
    ...
    try:
        ...
        resend_api_key = os.environ.get('RESEND_API_KEY')
        if not resend_api_key:
            return {"status": "skipped", "message": "Email service not configured", "sent": 0}
        ...
        if not settings or not settings.get("enabled", True):
            return {"status": "skipped", "message": "Reminders disabled", "sent": 0}
        
        days_before = settings.get("days_before", 1)
        event_types = settings.get("event_types", ["court_date", "attorney_meeting", "visitation", "medical"])
        ...
        if not events:
            return {"status": "success", "message": "No upcoming events to remind", "sent": 0}
        ...
        for event in events:
            ...
            already_sent = await db.sent_reminders.find_one({
            ...
            event_type_labels = {
                ...
                "child_support": "Child Support Court",
                "other": "Event"
            }
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <h2 style="color: #2C3E50;">Upcoming {event_label} Reminder</h2>
                <p>This is a reminder that you have an upcoming event:</p>
                <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
                    <h3 style="color: #1A202C; margin: 0 0 10px 0;">{event["title"]}</h3>
                    <p style="margin: 5px 0;"><strong>Date:</strong> {event["start_date"]}</p>
                    <p style="margin: 5px 0;"><strong>Type:</strong> {event_label}</p>
                    {f'<p style="margin: 5px 0;"><strong>Location:</strong> {event.get("location", "")}</p>' if event.get("location") else ''}
                    {f'<p style="margin: 5px 0;"><strong>Notes:</strong> {event.get("notes", "")}</p>' if event.get("notes") else ''}
                </div>
                <p style="color: #718096; font-size: 12px;">
                    This reminder was sent by CustodyKeeper. 
                    <a href="https://shared-custody.preview.emergentagent.com/settings">Manage reminder settings</a>
                </p>
            </div>
            """
            ...
            params = {
                ...
                "subject": f"Reminder: {event['title']} on {event['start_date']}",
                "html": html_content
            }
            ...
            try:
                ...
                logger.error(f"Failed to send reminder for event {event['event_id']}: {str(e)}")
        ...
        return {"status": "error", "message": "Email service not available", "sent": 0}
    except Exception as e:
        logger.error(f"Failed to send reminders: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to send reminders: {str(e)}")


# System-level endpoint to process all users' reminders (for cron job)
@api_router.post("/reminders/process-all")
async def process_all_reminders():
    """
    Process reminders for all users with enabled settings.
    This endpoint should be called by a scheduled cron job.
    Note: In production, add API key authentication for this endpoint.
    """
    try:
        import resend
        ...
        resend_api_key = os.environ.get('RESEND_API_KEY')
        if not resend_api_key:
            return {"status": "skipped", "message": "Email service not configured", "processed": 0}
        
        resend.api_key = resend_api_key
        sender_email = os.environ.get('SENDER_EMAIL', 'onboarding@resend.dev')
        
        # Get all users with enabled reminders
        users_with_settings = await db.reminder_settings.find(
            {"enabled": True},
            {"_id": 0}
        ).to_list(1000)
        
        # Also include users without settings (default is enabled)
        all_users = await db.users.find({}, {"_id": 0, "user_id": 1, "email": 1}).to_list(10000)
        users_with_settings_ids = {s["user_id"] for s in users_with_settings}
        ...
        for user in all_users:
            ...
            settings = next((s for s in users_with_settings if s["user_id"] == user_id), None)
            days_before = settings.get("days_before", 1) if settings else 1
            event_types = settings.get("event_types", ["court_date", "attorney_meeting", "visitation", "medical"]) if settings else ["court_date", "attorney_meeting", "visitation", "medical"]
            
            # If user has explicit settings with enabled=False, skip
        ...
        logger.error(f"Failed to process reminders: {str(e)}")
